...
bar_width = 0.8
bars1 = plt.bar(cleaned_ids, df['model_memory_mb'], bar_width, 
                label='Model Memory', color='skyblue')
bars2 = plt.bar(cleaned_ids, df[f'{DATASET}_descriptor_memory_mb'], bar_width,
                bottom=df['model_memory_mb'], label='Descriptor Memory', color='lightcoral')
...
for i, (height, score) in enumerate(zip(total_heights, df[f'{DATASET}_R@1'])):
    plt.text(i, height, f'R@1: {score:.1f}%', 
