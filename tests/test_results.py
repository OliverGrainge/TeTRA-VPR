...
def test_results_completeness(backbone, desc_divide_factor, agg_arch, image_size):
    ...
    model_id = (
        f"{backbone}_{agg_arch}_{image_size}_DescDividerFactor[{desc_divide_factor}]"
    )
    ...
    if model_id not in df["id"].values:
        ...
        for dataset in datasets:
            if (
                f"{dataset}_cosine_R@1" not in df.columns
                or df.loc[df["id"] == model_id, f"{dataset}_cosine_R@1"].isnull().all()
            ):
                print(
                    f"Missing results for {model_id} in dataset: {dataset}_cosine_R@1"
                )
            if (
                f"{dataset}_hamming_R@1" not in df.columns
                or df.loc[df["id"] == model_id, f"{dataset}_hamming_R@1"].isnull().all()
            ):
                print(
                    f"Missing results for {model_id} in dataset: {dataset}_hamming_R@1"
                )
