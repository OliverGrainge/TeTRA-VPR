...
class ViT(nn.Module):
    def __init__(
        ...
        model_type = (
            "VittinyT_partial"
            if self.dim == 192
            else "VitsmallT_partial" if self.dim == 384 else "VitbaseT_partial"
        )
...
def VittinyT_partial(image_size=[224, 224]):
    return ViT(
...
def VitsmallT_partial(image_size=[224, 224]):
    return ViT(
...
def VitbaseT_partial(image_size=[224, 224]):
    return ViT(
        image_size=image_size[0],  # Smaller image size for reduced complexity
        patch_size=14,
        dim=768,
