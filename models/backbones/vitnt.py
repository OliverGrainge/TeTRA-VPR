...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.0):
        ...
        self.rnorm1 = nn.RMSNorm(dim)
        self.rnorm2 = nn.RMSNorm(inner_dim)
...
class ViT(nn.Module):
    ...
    def __str__(self):
        model_type = (
            "VitsmallNT"
            if self.dim == 384
            else "VitbaseNT" if self.dim == 768 else "Vit"
        )
        return f"{model_type}{self.image_size}"


def VitsmallNT(image_size=[224, 224]):
    return ViT(
...
def VitbaseNT(image_size=[224, 224]):
    return ViT(
        image_size=image_size[0],  # Smaller image size for reduced complexity
        patch_size=14,
        dim=768,
