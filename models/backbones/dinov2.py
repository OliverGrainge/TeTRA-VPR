...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, return_attn=False) -> Tensor:
        def attn_residual_func(x: Tensor, return_attn=False) -> Tensor:

            if return_attn:
                x = self.norm1(x)
                x, attn = self.attn(x, return_attn=return_attn)
                x = self.ls1(x)
                return x, attn
            else:
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            if return_attn:
                ...
                x_attn, attn_map = attn_residual_func(x, return_attn=True)
                x = x + x_attn
...
class NestedTensorBlock(Block):
    ...
    def forward(self, x_or_x_list, return_attn=False):
        if isinstance(x_or_x_list, Tensor):
            return super().forward(x_or_x_list, return_attn=return_attn)
...
def _make_dinov2_model(
    ...
    **kwargs,
    ...
    # Create a dictionary mapping architecture names to their functions
    vits = {
        "vit_base": vit_base,
    ...
    model = vits[arch_name](**vit_kwargs)
...
class DinoV2(torch.nn.Module):
    ...
    def __init__(
        ...
        self.dino = dinov2_vitb14(pretrained=True, weights=Weights.LVD142M)
    ...
    def forward_distill(self, x, return_attn=False):
        ...
        if return_attn:
            return x, torch.stack(attention).permute(1, 0, 2, 3, 4)
        else:
            return x
...
def DinoV2_BoQ_Backbone(normalize=True):
...
def bitlinear_quantize_model(model: nn.Module) -> nn.Module:
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            replacements[name] = BitLinear.from_linear(module)
    
    # Then replace them after iteration is complete
    for name, new_module in replacements.items():
        # Split the name into parts to handle nested modules
        parts = name.split('.')
        parent_module = model
...
def DinoV2_BoQ_Backbone_Ternary(normalize=True):
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(
        io.StringIO()
    ):
        model = get_trained_boq(
