...
DATASET_ROOT = os.path.join(config["Datasets"]["datasets_dir"], "inria_holidays/")
GT_ROOT = config_path = os.path.join(os.path.dirname(__file__), "../../datasets/")

path_obj = Path(DATASET_ROOT)
if not path_obj.exists():
    raise Exception(
        f"Please make sure the path {DATASET_ROOT} to Inria dataset is correct"
    )
...
class InriaDataset(Dataset):
    def __init__(self, input_transform=None):
        ...
        self.ground_truth = np.load(
            GT_ROOT + "/InriaHolidays/Inria_gt.npy", allow_pickle=True
        )
