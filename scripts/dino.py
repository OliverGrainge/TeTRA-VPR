    ...
    def __init__(
        ...
        self.local_crop = T.Compose(
            [  # T.RandomRotation(degrees=10), #T.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(0.8, 1.2), shear=10),
                T.RandomResizedCrop(
                    (image_size, image_size), scale=(0.6, local_upper_crop_scale)
                )
            ]
        )
        ...
        self.teacher_encoder = NetWrapper(
            teacher_net,
            num_classes_K,
            projection_hidden_size,
            projection_layers,
...
class DinoSSL(pl.LightningModule):
    def __init__(
        ...
        image_size=224,
        hidden_layer=-2,
        projection_hidden_size=224,
        projection_layers=4,
        num_classes_K=65336,
        student_temp=0.9,
        teacher_temp=0.04,
        local_upper_crop_scale=0.8,
        global_lower_crop_scale=0.8,
        moving_average_decay=0.9,
        center_moving_average_decay=0.9,
    ):
        super().__init__()
        ...
        self.total_images = self.count_total_images(
            os.path.join(self.dataset_dir, "shard_*.tar")
        )
    ...
    def train_dataloader(self):
        ...
        shards = os.path.join(
            self.dataset_dir, "shard_{000000.." + f"{n_shards:06d}" + "}.tar"
        )
        dataset = (
            wds.WebDataset(shards)
            .decode("pil")
            .to_tuple("jpg")
            .map(lambda x: (transform(x[0]),))
        )
...
module = DinoSSL(
    model,
    dataset_dir="/home/oliver/datasets_drive/vpr_datasets/sf_xl/compressed/dataset",
)
