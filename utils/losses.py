...
from .distances import HammingDistanceSoft, binarize_ste, HammingDistanceSTE
...
class GreedyHashLoss_Soft(nn.Module): 
    def __init__(self, alpha=1.0, beta=50, base=0.0, reg=1.0): 
        ...
        self.base = base 
        self.reg = reg 
        self.loss_fn = losses.MultiSimilarityLoss(alpha=alpha, beta=beta, base=base, distance=HammingDistanceSoft())
        self.mse = nn.MSELoss()
...
class GreedyHashLoss(nn.Module): 
    def __init__(self, alpha=1.0, beta=50, base=0.0, reg=1.0): 
        ...
        self.base = base 
        self.reg = reg 
        self.loss_fn = losses.MultiSimilarityLoss(alpha=alpha, beta=beta, base=base, distance=HammingDistanceSTE())
        self.mse = nn.MSELoss()
...
def get_loss(loss_name):
    ...
    if loss_name == "GreedyHashLoss_soft": return GreedyHashLoss_Soft(alpha=1.0, beta=50, base=0.0, reg=1.0)
    if loss_name == "GreedyHashLoss": return GreedyHashLoss(alpha=1.0, beta=50, base=0.0, reg=1.0)
...
def get_miner(miner_name, margin=0.1):
    ...
    if miner_name == 'HammingSTEMultiSimilarityMiner' : return miners.MultiSimilarityMiner(epsilon=margin, distance=HammingDistanceSTE())
    if miner_name == 'HammingSoftMultiSimilarityMiner' : return miners.MultiSimilarityMiner(epsilon=margin, distance=HammingDistanceSoft())
